import { KSWAP } from './../../constants/index'
import { TokenAmount } from '@kimchiswap/sdk'
import { isAddress } from 'ethers/lib/utils'
import { useGovernanceContract, useKswapContract } from '../../hooks/useContract'
...
import { abi as GOV_ABI } from '@kimchiswap/governance/build/GovernorAlpha.json'
...
export function useUserDelegatee(): string {
  ...
  const kswapContract = useKswapContract()
  const { result } = useSingleCallResult(kswapContract, 'delegates', [account ?? undefined])
...
export function useUserVotes(): TokenAmount | undefined {
  ...
  const votes = useSingleCallResult(kswapContract, 'getCurrentVotes', [account ?? undefined])?.result?.[0]
  return votes && kswap ? new TokenAmount(kswap, votes) : undefined
}
...
export function useUserVotesAsOfBlock(block: number | undefined): TokenAmount | undefined {
  ...
  // check for available votes
  const kswap = chainId ? KSWAP[chainId] : undefined
  const votes = useSingleCallResult(kswapContract, 'getPriorVotes', [account ?? undefined, block ?? undefined])
    ?.result?.[0]
  return votes && kswap ? new TokenAmount(kswap, votes) : undefined
}
...
export function useDelegateCallback(): (delegatee: string | undefined) => undefined | Promise<string> {
  ...
  return useCallback(
    (delegatee: string | undefined) => {
      ...
      if (!kswapContract) throw new Error('No KSWAP Contract!')
      return kswapContract.estimateGas.delegate(...args, {}).then(estimatedGasLimit => {
        return kswapContract
    ...
    [account, addTransaction, chainId, library, kswapContract]
  )
}
