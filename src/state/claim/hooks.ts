...
import { TokenAmount, JSBI, ChainId } from '@kimchiswap/sdk'
...
// check if user is in blob and has not yet claimed KSWAP
...
export function useUserUnclaimedAmount(account: string | null | undefined): TokenAmount | undefined {
  ...
  if (!canClaim || !userClaimData) {
    return new TokenAmount(kswap, JSBI.BigInt(0))
  }
  return new TokenAmount(kswap, JSBI.BigInt(userClaimData.amount))
}
      ...
      return distributorContract
        ...
        .then((response: TransactionResponse) => {
          addTransaction(response, {
            summary: `Claimed ${unClaimedAmount?.toSignificant(4)} KSWAP`,
